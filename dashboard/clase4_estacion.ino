...
void init_sensores(Sensor s[]) {
  strcpy(s[0].nombre, "DHT11_TEMP");
  s[0].pin = 2;
  s[0].tipo = TIPO_DIGITAL;
  s[0].valor = 0;
  s[0].umbral_alerta = 30;
  s[0].umbral_critico = 35;
  s[0].activo = true;
  s[0].offset = 0;
  strcpy(s[1].nombre, "DHT11_HUM");
  s[1].pin = 2;
  s[1].tipo = TIPO_DIGITAL;
  s[1].valor = 0;
  s[1].umbral_alerta = 80;
  s[1].umbral_critico = 90;
  s[1].activo = true;
  s[1].offset = 0;
  strcpy(s[2].nombre, "LM35");
  s[2].pin = A0;
  s[2].tipo = TIPO_ANALOGICO;
  s[2].valor = 0;
  s[2].umbral_alerta = 30;
  s[2].umbral_critico = 35;
  s[2].activo = true;
  s[2].offset = 0.3;
  strcpy(s[3].nombre, "LDR");
  s[3].pin = A1;
  s[3].tipo = TIPO_ANALOGICO;
  s[3].valor = 0;
  s[3].umbral_alerta = 600;
  s[3].umbral_critico = 800;
  s[3].activo = true;
  s[3].offset = 0;
  strcpy(s[4].nombre, "LLUVIA");
  s[4].pin = A2;
  s[4].tipo = TIPO_ANALOGICO;
  s[4].valor = 0;
  s[4].umbral_alerta = 500;
  s[4].umbral_critico = 800;
  s[4].activo = true;
  s[4].offset = 0;
}
...
void leer_sensor(Sensor *s) {
  ...
  if (s->tipo == TIPO_ANALOGICO) {
    ...
    if (strcmp(s->nombre, "LM35") == 0)
      s->valor = (raw * 5.0 / 1023.0) * 100.0 + s->offset;
    else
      s->valor = raw;
  } else {
...
void leer_todos(Sensor s[], int n) {
  for (int i = 0; i < n; i++)
    if (s[i].activo)
      leer_sensor(&s[i]);
}
...
void loop() {
  if (Serial.available()) {
    char c = Serial.read();
    if (c == 'R') {
      Sensor *p = buscar_sensor(sensores, 5, "LM35");
      if (p) {
        leer_sensor(p);
        Serial.print("AHORA ");
        Serial.print(p->nombre);
        Serial.print(",");
        Serial.println(p->valor, 1);
      }
    } else if (c == 'B') {
      tone(BUZZER_PIN, 2000);
      delay(200);
      ...
      for (int i = 0; i < 5; i++) {
        ...
        Serial.print(" ");
        Serial.print(sensores[i].valor, 1);
        Serial.print(" pin");
        Serial.println(sensores[i].pin);
      }
    }
  }
  if (millis() - ultima >= INTERVALO_MS) {
    ...
    if (t && h) {
      Serial.print(t->valor, 1);
      Serial.print(",");
      Serial.print(h->valor, 0);
      Serial.print(",");
      Serial.print(ldr ? ldr->valor : 0, 0);
      Serial.print(",");
      Serial.println(agua ? agua->valor : 0, 0);
    }
    ultima = millis();
  }
}
